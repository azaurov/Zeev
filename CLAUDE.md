...
**Russian TTS routes to bosgame, not local Piper**: measured Russian Piper directly on both machines — Pi Zero 2W ARM core: RTF ~2.6x (a 4-5 word reply took 18-20s); bosgame's x86 CPU with the identical model: RTF ~0.135x, ~1s total for the same phrase. The model isn't the bottleneck, the Pi's CPU is. `_speak_device()` step 0 now tries `_audio.speak_sync(lang="ru")` first (daemon → bosgame's `tts_server.py`, which routes `"lang":"ru"` to a dedicated Piper model instead of Kokoro, which doesn't support Russian) and only falls to local `_piper_direct()` (step 0b) if the daemon is unavailable or fails. `audio_client.speak_sync()` returns `bool` (the daemon's `ok` field) so this fallback can actually detect failure. feiergente01 (the second English backend) doesn't have the Russian model — `speakPiper` in the Go daemon disables the two-backend chunk split for `lang="ru"`, keeping all Russian chunks on bosgame. `PIPER_MODELS["ru"]` (local fallback only) prefers `ru_RU-irina-medium.onnx` (female) over `ru_RU-dmitri-medium.onnx` if both are present (`init_tts()`).
...
- **Server**: `~/piper/tts_server.py` (port 5600, localhost-only). Service: `piper-tts.service`. Request `"lang":"ru"` routes to `~/piper/ru_RU-irina-medium.onnx` via Piper instead of Kokoro (Kokoro doesn't support Russian); any other/no lang uses Kokoro with English Piper as its own fallback on error.
