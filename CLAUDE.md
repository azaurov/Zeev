...
- **LCD "speaking" state is an 8-band equalizer visualizer** (`face_scroll.py` `_draw_equalizer`), not the Miss Minutes face, for every spoken reply (all other states keep Miss Minutes). Two independent sources feed real per-band levels into `_eq_levels[0]`, and `eq_levels=None` draws a bounded synthetic per-bar sine pattern when neither has anything (`_draw_equalizer`'s fallback, same real-or-synthetic contract `mouth_shape=None` already used for the flap animation):
