...
- **Same-morning birthdays are merged into one sentence** (`gcal_plan_all` → `gcal_birthday_text`). The imported **`Bdays`** calendar is 154/year over 119 days — 21 days carry two, 4 carry three, 2 carry four — so clustering is normal, not an edge case, and with a birthday most days one date's day-ahead lands on the same 9 AM as another's day-of. Result: *"Two birthdays today: Rachel Zaurov and Oscar Florez. And tomorrow: Bruno Costa, Al Chemist and Avraham Moshe Goldberg."* Membership is **hashed into the key**, so removing one prunes the group instead of leaving "Four birthdays" surviving as three.
  - **The digest must change only when the CALENDAR changes, never as the clock moves.** Live 2026-08-02: 09:00 announced *"Two birthdays today: Sabrina Escalante and Nathan Is. And tomorrow: RJ Megesi"*, then the **09:09 scan re-announced "RJ Megesi's Birthday is tomorrow"** — Sabrina and Nathan had dropped out of the digest the moment their own 9 AM passed, minting a new key for the same bucket. `gcal_plan_all` now accumulates membership **unconditionally** and dues **conditionally**, so a passed birthday stays in its bucket's digest for the rest of the day; a bucket with no remaining due is skipped, then pruned. `_gcal_due_for_lead` also returns `None` for an event already under way — without it, admitting passed birthdays would let them clamp a due to `now` and announce themselves.
