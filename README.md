...
Hold the KEY button to record, release to send. Press again while Zeev is speaking to interrupt. The LCD shows an animated face that reflects the current state (idle / listening / thinking / speaking). Speaker volume is set to 60% via `amixer` at startup.
