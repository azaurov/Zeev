...
- **`set_volume(level)` / `get_volume()`** — get/set system volume (0–100). `set_volume()` calls `amixer sset Master N%`; if that fails, falls back to `amixer -c wm8960soundcard sset Speaker` (raw 0–127). State stored in `_VOLUME` global (default 60).
...
- **Speaker volume**: set to 60% via `amixer` at startup (`hw:wm8960soundcard`, `Speaker` control, raw range 0–127)
