...
- **Wake word ("Miss Minutes")**: `_wake_listener()` picks one of two front-ends, both feeding the shared `_wake_dispatch()` (session claim → beep → dispatch or record follow-up), so the paths can't drift on state handling.
  - **openWakeWord (preferred)**: one continuous 16kHz raw `arecord` stream, scored on-device per 1280-sample (80ms) frame. Enable with `OWW_MODEL_PATH` in `.env`; `OWW_THRESHOLD` (0.5) and `OWW_COOLDOWN` (2.0s) tune it. Install `onnxruntime` + `openwakeword`.
    ...
    - **`OWW_THRESHOLDS`** (`stem:0.80,stem:0.55`) overrides the global `OWW_THRESHOLD` per model; unlisted stems keep the global. Thresholding happens **per model before the max is taken** (`oww_best`), or a model with a raised bar would mask a quieter one that legitimately fired. Measured live 2026-07-29 over 8h of household conversation, though: `sarina`'s real wakes spanned 0.58–0.98 and its false wakes 0.51–0.73 — **overlapping, so no threshold separates them**. ~0.7 fp/hr on fluent speech against upstream's 0.2 target is a phrase-length problem; the fix is retraining as "Hey Sarina", not the knob. See `docs/wake-word-training.md`.
