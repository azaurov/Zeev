...
- **Wake word ("Miss Minutes")**: `_wake_listener()` picks one of two front-ends, both feeding the shared `_wake_dispatch()` (session claim → beep → dispatch or record follow-up), so the paths can't drift on state handling.
  - **Porcupine (preferred)**: one continuous 16kHz raw `arecord` stream scored on-device per 512-sample frame. Measured on the Pi Zero 2W: 100% of realtime, **0.7% of one core**, zero API spend when idle. Needs `PORCUPINE_ACCESS_KEY` + `PORCUPINE_KEYWORD_PATH` (a custom `.ppn` built on the Picovoice console for the *raspberry-pi* platform) in `.env`, plus `pip3 install --user --break-system-packages pvporcupine`. openwakeword was rejected: it ships no "Miss Minutes" model, and training one wants a GPU.
  - **Cloud fallback**: used automatically when the key/`.ppn` is missing or the model won't load. Records 2s chunks, applies an RMS gate then `_has_speech()`, and only then pays for Groq Whisper and substring-matches `_DEVICE_WAKE_WORDS`. **This bills per noisy 2s window, continuously** — it is a fallback, not a resting state.
  - **Behavior difference**: the cloud path can catch "Miss Minutes, what's the weather" in one window and pass the tail straight through; Porcupine fires on the phrase alone, so the follow-up is always recorded separately. Speak, pause, then ask.
