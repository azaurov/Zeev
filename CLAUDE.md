...
- **`news_probe.py`** — same shape as `rag_probe.py`: grades a stored digest's summary against its own stored snippets via a fresh-context LLM call, logs to `news_probes` (`digest_id`, `grounded`, `grader_note`), `--report` shows the rolling grounded rate. First real run (once the reasoning-effort fix landed) graded GROUNDED 1/1.
...
- **LCD face is `face_aura.py`** (2026-08-11, replaced `face_scroll.py`'s Miss Minutes character + separate equalizer bars): one continuous visual language across every state — a pulsing orb with 8 radiating spokes, reusing `face_scroll`'s header/clock/battery/scrolling-text code untouched, only the icon area changed. Per-state spoke pattern (`_band_levels`): idle/ready slow uniform breathing, listening a gentle irregular murmur, thinking a single spoke sweeping the ring (radar-style), error all spokes strobing + orb jitter, speaking real per-band audio levels. `face_scroll.py` is left in place and still importable (drop-in-compatible `draw_frame` signature) but is no longer the live renderer — swap back by changing the one `from face_aura import draw_frame` line in `_face_loop()`. `preview_faces.py` renders all four styles (`face`/`wave`/`scroll`/`aura`) side by side to `zeev/previews/grid_*.png` without touching hardware; `face_states_test.py` cycles the live renderer on real hardware. Rendering cost matched to what it replaced (a handful of nested ellipse fills + short lines, no per-frame resize/alpha compositing) given the RGB565-over-SPI push already needed a perf fix once (see below).
- **Speaking-state audio levels** feed `_eq_levels[0]`, which `face_aura._draw_aura` reads for `state == "speaking"` (falls back to a bounded synthetic per-band sine when nothing live is available — same real-or-synthetic contract `mouth_shape=None` already used for the old lipsync flap animation, which `face_aura` accepts but ignores). Two independent sources feed it:
