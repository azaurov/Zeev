...
- **Wake word ("Miss Minutes")**: `_wake_listener()` picks one of two front-ends, both feeding the shared `_wake_dispatch()` (session claim → beep → dispatch or record follow-up), so the paths can't drift on state handling.
  - **openWakeWord (preferred)**: one continuous 16kHz raw `arecord` stream, scored on-device per 1280-sample (80ms) frame. Enable with `OWW_MODEL_PATH` in `.env`; `OWW_THRESHOLD` (0.5) and `OWW_COOLDOWN` (2.0s) tune it. Install `onnxruntime` + `openwakeword`.
    ...
    - **Thermal context**: idle Pi with zeev-device stopped sits at **58 °C**; running it is ~59–63 °C, and 65.8–68.8 °C at the hotter wake-gate settings. Throttling starts at 80 °C and the sticky `vcgencmd get_throttled` register has stayed `0x0` — it has never actually throttled.
      - **Every one of those figures is measured inside a closed 3D-printed enclosure** (Pwnagotchi keychain type: solid shell, display window, port cutouts on one face). A bare Zero 2W in open air is ~40–45 °C, so the gap is the HAT/PiSugar sandwich *and* the sealed case with no airflow — do not compare these against bare-board numbers.
      - **Print material is a real limit**: internal air sits at or above **PLA's glass transition (~60–65 °C)**, so a PLA case can soften over months of continuous running. PETG (~80 °C) or ABS/ASA has margin. Warping around the display window or port cutouts would be this, not board damage.
      - The case also **muffles the mic and speaker**, which is why wake/whisper RMS figures here are not comparable to published openWakeWord numbers, and part of why high volume settings felt necessary before the dB recalibration.
