...
- **The bridge is blocked on hardware, not code**: cameras report `dtls: 1` and the bridge times out on the TUTK handshake (`IOTC_ER_TIMEOUT`) — a known open upstream issue. The seven un-flashed cameras are still stuck behind it.
