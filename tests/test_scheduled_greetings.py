...
def test_silent_on_saturday(zeev):
    assert zeev._due_greetings(SAT_700, {}) == []
...
def test_no_repeat_same_day(zeev):
    ...
    last_fired = {"morning": MON_700.date()}
    due = dict(zeev._due_greetings(MON_700, last_fired))
    assert "morning" not in due


def test_fires_again_next_weekday(zeev):
    tue_700 = _dt(2026, 8, 18, 7, 0)
    last_fired = {"morning": MON_700.date()}
    due = dict(zeev._due_greetings(tue_700, last_fired))
    assert "morning" in due
