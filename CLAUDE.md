...
- **LCD "speaking" state is an 8-band equalizer visualizer** (`face_scroll.py` `_draw_equalizer`), not the Miss Minutes face, for every spoken reply (all other states keep Miss Minutes). Real per-band levels come from `zeev.py`'s `_play_pcm_chunked` (same FFT chunk already used for lipsync's spectral centroid, log-spaced into 8 bands, each normalized against its own slow-decaying peak) into `_eq_levels[0]`, reset to `None` in the same `finally` block that resets `_mouth_shape[0]`. **That path only runs for the Orpheus/BT-fallback TTS route** — the primary route, the Go daemon's `speak_sync()`, plays audio entirely inside the daemon and hands nothing back to Python, so most turns have no live signal at all. `eq_levels=None` draws a bounded synthetic per-bar sine pattern instead (`_draw_equalizer`'s fallback), same real-or-synthetic contract `mouth_shape=None` already used for the flap animation. Getting the daemon path wired to real levels too would need the Go daemon to publish live PCM/RMS back over the Unix socket — not done.
