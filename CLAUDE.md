...
- **"check all cameras" reaches the ask path, not a sweep.** `resolve_wyze_cam()` returns a single stream; there is no multi-camera intent. Widening the regex did not create one.
